...
def load_and_clean():
    ...
    tech_reviews = pd.read_csv('data/data_1_test.csv', quoting=csv.QUOTE_NONE, error_bad_lines=False, skipinitialspace=True)
    food_reviews = pd.read_csv('data/data_2_test.csv', quoting=csv.QUOTE_NONE, error_bad_lines=False)

    # rename columns to remove whitespaces
    tech_reviews.columns = ['example_id', 'text', 'aspect_term', 'term_location']
    food_reviews.columns = ['example_id', 'text', 'aspect_term', 'term_location']
